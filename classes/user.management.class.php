...
class User extends MySQL
{
        ...
        if ($rec_count == 1) {
            ...
            if ($result) {
                ...
                while (!$this->EndOfSeek()) {
                    $row = $this->Row();
                    $uclist .= "<tr><td>" . $row->cat_name . "</td><td>".$row->status."</td><td><a href=\"#\" id=\"delete\" data-id=\"$row->cat_id\" class=\"btn btn-danger\"><i class=\"fa fa-times\"></i> Delete</a></td></tr>";
                }
                $uclist .= "</tbody></table>";
            ...
            $whereArray['cat_id'] = MySQL::SQLValue($id);

            $tableName = 'usr_cat';

            $sql = MySQL::BuildSQLDelete($tableName, $whereArray);
            $result = $this->Query($sql);
    ...
    function addUser($firstname,$lastname,$phone,$email,$status,$category){


        $login = trim($email);
        $password = sha1(trim($email));
        $usercat = $category;
        ...
        if($check_result > 0){
            ...
            $valuesArray['username'] = MySQL::SQLValue($email);
            $valuesArray['status'] = MySQL::SQLValue($status);
            $valuesArray['created_by'] = $_SESSION['hems_User']['user_id'];
            $valuesArray['created_on'] = MySQL::SQLValue(date('Y-m-d:i:s'));
    ...
    function editUser($firstname,$lastname,$phone,$email,$status,$category,$id){
        ...
        $usercat = $category;
            ...
            $valuesArray['username'] = MySQL::SQLValue($email);
            $valuesArray['status'] = MySQL::SQLValue($status);
            $valuesArray['updated_by'] = $_SESSION['hems_User']['user_id'];
            $valuesArray['updated_on'] = MySQL::SQLValue(date('Y-m-d:i:s'));
            $whereArray['user_id'] = MySQL::SQLValue($id);

            $sql = MySQL::BuildSQLUpdate("usr_users", $valuesArray,$whereArray);
            $result = $this->Query($sql);
    ...
    function findUser($uid){
        ...
        if($counting > 0){
            ...
            while(!$this->EndOfSeek()){ $row = $this->Row();
                $ulist .= "<tr><td>".$row->first_name.' '.$row->last_name."</td><td>".$row->username."</td><td>".$row->created_on.'</td><td><a href="#viewuserinfo" data-toggle="modal" id="userdetail" name='.$row->user_id.'>View</a></td><td><a href="edit_user.php?id='.base64_encode($row->user_id).'" data-toggle="modal" id="euserdetail" name='.$row->user_id.'>Edit</a></td>';
    ...
    public  function setPassword($username,$password){
            $valuresArray['password'] = MySQL::SQLValue(sha1($password));
            $whereArray['username'] = MySQL::SQLValue($username);
            $sql= MySQL::BuildSQLUpdate('usr_users',$valuresArray,$whereArray);
            $check =  $this->Query($sql);
            if($check){return 'ok';}else{return 'fail';}
    }
}